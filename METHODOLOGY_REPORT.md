...
| Date | Changes |
|------|--------|
| 23 Aug 2026 | **`--bt-gap-pool`: weighted position pooling for the Barlow Twins GAP term, replacing the uniform mean.** The GAP companion exists because writing patch features as `z[n,p,c] = s[n,c] + r[n,p,c]` makes the uniform mean recover the subject term `s` *exactly* — `r` is defined as the deviation from that mean, so it sums to zero over positions by construction. What the uniform mean does badly is **dilute focal signal**: a factor living in a few positions of P contributes ~1/P of its local magnitude, which is the suspected cause of `gap_feat_std` parking at ~0.004 against a hinge target of 1 (the measurement that forced `--bt-gap-std-coeff` to be split from `--bt-std-coeff`). Two modes, default `mean` so every existing run is bit-identical. **`variance`** (no parameters, detached) weights positions by across-subject variance, normalising each (view, channel) profile over positions before averaging so one high-amplitude channel cannot set the profile — the `--bt-sim-normalize` direction-equalisation argument. **`learned`** fits one logit per position, softmax over positions, zero-initialised so it *starts* exactly at the mean. **Both refuse to condition on the sample, and that is the design, not a simplification**: a sample-conditioned attention module revives the interaction term the GAP pooling exists to integrate away (`sum_p a_{n,p} r_{n,p}` is then correlated with the features rather than vanishing), *and* hands the loss a shortcut — put mass where the two views already agree, which `on_diag` rewards for free given it already reaches ~1 on shared anatomy alone. Weights are also shared across views, or `on_diag` reads "the two weightings agreed" as "the content agreed". **What this does not do:** the correlation still has B subject rows, so `off_diag` keeps its `d(d-1)/B` floor untouched — this raises signal, not sample count, and composes with `--bt-corr-ema` rather than replacing it. **Measured on planted data** (4 focal positions of 256, interaction noise at signal scale): cross-view correlation of the pooled feature 0.101 → 0.230 under `variance`. **Known limit, recorded so nobody rediscovers it from a dead run:** the entropy floor guarding `learned` against collapse has gradient `dH/dw_j = -a_j(log a_j + H)`, which *vanishes* as `a_j → 1` — measured 0.70 in the active band (H/logP 0.23) but 3e-6 once saturated, so a fully collapsed softmax is an absorbing state, the same shape as the collapsed projection head. It is caught on the way down, not recovered from; watch `Contrastive/gap_pool_entropy_L*` and restart from an earlier checkpoint rather than raising the coefficient mid-run. Verify with `python -m scripts.smoke_bt_gap_pool`; ablations in `experiments/gap_pool_{variance,learned}.yaml`, which mirror `synthetic_causal.yaml` line-for-line because `launch.py` deliberately does not chain `_base_`. **Run `variance` first — if it does not move `gap_feat_std`, `learned` is fitting noise.** |
