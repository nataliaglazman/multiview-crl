...
| Date | Changes |
|------|--------|
| 3 Sep 2026 | **`--bt-gap-pool per_channel` — measured on a real checkpoint, and it fails the SAME way the shared modes did, despite having C×P parameters to escape the constraint that killed them.** The 23 Aug entry rejected `learned`/`variance` because per-factor profiles are mutually orthogonal, and left `per_channel` open as "the version of this idea that could work." Read back from `results/synthetic/gap-pool-per-channel-rescaled` (step 99001, `--bt-gap-pool-coarsen 2`, `--patch-foreground-mask`) with `eval/gap_pool_profile.py --from-checkpoint` (new this session — reads the trained `_bt_gap_pools.L{lvl}` tensors directly out of the checkpoint, since `load_model_from_run_dir` has no attachment point for them and silently drops them into `unexpected_keys`). **It concentrated for real:** `gap_pool_entropy` 0.694 (of 1.0 uniform), `max_weight` 27.6× uniform, `channel_agreement` 0.083 (channels read different places, as intended), `coverage` 0.893. **But against its own permutation null it bought nothing** — the same weight values reassigned to random positions score the same cross-view correlation, z = 0.1 — **and `on_diag` went 0.190 → 1.202, ~6× WORSE than plain mean pooling**, a scale-invariant correlation so not a `feat_std` artifact. **Mechanism:** the learned profile's centred cosine with the across-subject-variance profile is **+0.393** — it converged onto the same variance-tracks-style positions the 23 Aug entry already measured as anti-aligned with content (z = −4.4 on the recon baseline). Each channel independently re-discovered the same wrong signal; the extra parameters bought diversity of *where*, not correctness of *where*. Per-factor cosines on this checkpoint are **not usable** — split-half reliability is ~0 for every factor here, unlike the 60k checkpoints in the 23 Aug entry where `ventricle_size` read 0.662 at split-half 0.79 — only the variance-profile cosine is a direct enough statistic to trust. **`Weighted/bt_gap_pool_entropy_L*` reading ~0 for the whole run is not evidence the pooling stayed uniform:** the hinge is `relu(floor·log P − H)`, zero by construction above the floor, and concentration moved a long way (`eff_pos` 512 → ~40 per the foreground-masked TB curve) without ever crossing it — `gap_pool_entropy_L*` / `gap_pool_eff_pos_L*` are the curves that show movement; the hinge only shows overshoot past a threshold. Coarsening also weakens the stated floor: positions sharing a `--bt-gap-pool-coarsen 2` group share a logit, so position-space entropy cannot fall below `log(coarsen³)` regardless of what is learned, making `--bt-gap-pool-entropy 0.5` (a position-space number) only a **0.25** floor in the group units the pooler actually learns in (`eff_groups` 17.1 of 64 on this run); at coarsen 4 on an 8³ grid the floor is unreachable at any concentration. **Two caveats in the new readback itself, not yet fixed:** it reads the full grid while training only ever softmaxed over kept (foreground) positions, so its `eff_pos` (137 of 512) overstates concentration by ~3.4× against the logged curve — needs the run's keep-mask threaded through `pool.weights()`; and its printed `eff_pos` mixes `mean(exp(H))` against `mean(H)` rather than one consistent aggregation. **Confound not yet isolated:** this run's `--bt-gap-std-coeff` is inherited from `--bt-std-coeff` (0.00227) and the profile's hinge-state readout is PULLING (66% of 40 channels below target), so the GAP term's scale is being driven by that hinge and the pooling's reweighting at once. **Not yet checked against representation quality** — everything above is internal to the Barlow Twins loss; the next step is a `--bt-gap-pool mean` control at the same step compared on DCI/MCC. Code: `--from-checkpoint` (default on) added to `eval/gap_pool_profile.py`; regression coverage in `scripts/smoke_bt_gap_pool.py` sections 22–23; work lives on `claude/bt-gap-pool-entropy-ccnes0`, merged from `claude/bt-gap-explanation-fqxlyj` (the original `--bt-gap-pool` work, previously unmerged into `main`). |
...
| 23 Aug 2026 | **`--bt-gap-pool` — weighted position pooling for the Barlow Twins GAP term. Built, measured, and REJECTED, but for a reason worth keeping: the factors ARE spatially localised and they peak in DIFFERENT PLACES, so one shared position weighting cannot serve them.** **The result.** Measured on two step-60k synthetic checkpoints with `eval/gap_pool_profile.py`. Variance weighting fails on both — contrastive z = 0.7 against its own permutation, recon-only baseline z = **−4.4**, i.e. significantly WORSE, because the highest-across-subject-variance positions carry per-view style (the gain/bias transforms `style_leak_by_position.py` tracks) rather than shared content. Per-factor, under `--causal iid` with per-position null subtraction, localisation is unambiguous and anatomically correct: `brain_size` 0.120 and `cortical_thickness` 0.121 (top-10% mass, 0.10 = flat) sit at the global end, while `ventricle_size` reaches **0.662** (σ 56, split-half 0.79), `temporal_atrophy` **0.351** (σ 14, split-half 0.91), and `lesion_y/z` **0.74/0.80**. But their profiles are mutually orthogonal — mean pairwise cosine **+0.098** (contrastive) and **−0.056** (baseline). `--bt-gap-pool` implements ONE weight vector shared across every channel and factor, so mass moved toward the ventricles is mass taken from the lesions. Structural mismatch between instrument and data, not a tuning problem; the `learned` variant cannot fix it either. **Three measurement errors that produced a false negative first, all biasing toward 'flat'.** *(1)* The probe defaulted to `--causal match`, which this changelog already warns is invalid for per-factor attribution — under the random SCM `brain_size` is recovered at 0.92 and every factor with a recoverable parent inherits its flat global profile. Under `match`, `ventricle_size` read 0.113; under `iid`, 0.662. *(2)* No per-position null subtraction: a ridge on C features returns a positive R² by chance, that floor is constant across positions, and adding a constant flattens a profile. *(3)* A fixed 0.15 concentration threshold, calibrated on planted cases where top-10% mass reached 1.0. Replaced by a σ against the least-concentrated factor in the set, because top-10% takes the largest 10% of entries and so exceeds 0.10 in any finite sample — a planted GLOBAL factor reads 0.115 ± 0.001, which against a 0.10 reference is 22σ of nothing. **The other trap, recorded because the obvious fix is wrong:** a permutation null for profile CONCENTRATION fails, since shuffling labels drives R² to zero everywhere and an all-zeros profile with one surviving entry normalises to MAXIMAL concentration — the null then reads more localised than the data. Split-half reliability is the correct tool and is now reported per factor. **Also corrected: `gap_feat_std` cannot diagnose pooling.** It is set by the variance hinge (`relu(1 − std)`, no restoring force above 1), not by the pooling; 0.004 is the pre-hinge state and ~1.1 the parked one, and `on_diag`/`off_diag` are scale-invariant regardless. The original motivation for this whole flag was a misreading of that number. **Fallout worth acting on: no experiment YAML sets `bt_gap_std_coeff`**, so all inherit `bt_std_coeff`; check `Contrastive/gap_var_loss_L*` and the probe's hinge-state line (PULLING / BOUNDARY-ACTIVE / INERT). **What shipped.** `models.vqvae.GapPositionPool` (`variance`, detached; `learned`, zero-init so step 0 is exactly the old mean), default `mean` so every existing run is bit-identical; and `eval/gap_pool_profile.py`, which answers all of this from a checkpoint with no training run. Both refuse to condition on the sample by design — a sample-conditioned weight revives the interaction the pooling exists to integrate away and lets the loss put mass where the views already agree, which `on_diag` rewards for free. `learned`'s entropy floor has gradient `−a_j(log a_j + H)`, which vanishes as `a_j → 1`: measured 0.70 in the active band, 3e-6 once saturated, so full collapse is an absorbing state — caught on the way down, never recovered from. 16 regression sections in `scripts/smoke_bt_gap_pool.py`. |
