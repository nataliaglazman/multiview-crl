...
| Date | Changes |
|------|--------|
| 23 Aug 2026 | **`--bt-gap-pool` — weighted position pooling for the Barlow Twins GAP term. Built, tested, and MEASURED NOT TO HELP on this project's data. Default stays `mean`; the probe and the negative result are the deliverables.** **The hypothesis.** The GAP companion recovers the subject term `s` *exactly* — writing patch features as `z[n,p,c] = s[n,c] + r[n,p,c]`, `r` is defined as the deviation from the uniform mean and sums to zero over positions by construction. What the uniform mean does badly is **dilute focal signal**: a factor living in a few of P positions contributes ~1/P of its local magnitude. So weight the positions instead. **Rejected twice, on two step-60k synthetic checkpoints, by `eval/gap_pool_profile.py`.** On the CONTRASTIVE run the variance profile is only mildly peaked (top-10% mass 0.23) and weighting by it does not beat its own permutation (**z = 0.5**) — consistent with the 20 Aug finding that contrastive training moves content into channel identity rather than spatial layout (`gap` eff_rank 4.2 → 30.9), leaving no positional structure to grip. On the RECON-ONLY baseline the profile IS strongly peaked (top-10% mass **0.45**, max/uniform 7.27) and weighting by it is significantly **WORSE** than its permutation (**z = −3.7**, cross-view correlation 0.157 → 0.055): the highest-across-subject-variance positions carry per-view **style** — the global gain/bias transforms `style_leak_by_position.py` tracks — so upweighting them strips out cross-view agreement. Across-subject variance is a proxy for subject signal and on this data a *misleading* one. **The motivating measurement was also misread, and this is the transferable lesson.** The premise was that `gap_feat_std` parking at ~0.004 showed dilution. It does not: `feat_std` is set by the **variance hinge** (`relu(1 − std)`, no restoring force above 1, so it overshoots and parks), not by the pooling. 0.004 is the PRE-hinge state; a live run reads **~1.1** post-hinge. Since `on_diag`/`off_diag` are scale-invariant, `feat_std` cannot diagnose a pooling question at all. Every scale-invariant metric (cross-view correlation, factor R²) said NO from the start. **Fallout worth acting on independently: no experiment YAML sets `bt_gap_std_coeff`**, so all of them silently inherit `bt_std_coeff: 10` and run the exact failure mode that flag's own help documents — check `Contrastive/gap_var_loss_L*`; pinned at 0 means the term is inert while its per-channel inflation (which that help ties to a worse reconstruction plateau) stands. **What shipped.** `models.vqvae.GapPositionPool` with `variance` (detached, no parameters) and `learned` (one logit per position, softmax, zero-init so step 0 is exactly the old mean) modes. Both refuse to condition on the sample **by design**: a sample-conditioned weight revives the interaction the pooling exists to integrate away, *and* hands the loss a shortcut — put mass where the views already agree, which `on_diag` rewards for free given it reaches ~1 on shared anatomy alone. Weights are shared across views, or `on_diag` reads "the weightings agreed" as "the content agreed". Neither changes the number of ROWS, so the `d(d-1)/B` off-diagonal floor is untouched. **Known limit of the `learned` guard:** the entropy floor has gradient `dH/dw_j = −a_j(log a_j + H)`, which *vanishes* as `a_j → 1` — measured 0.70 in the active band (H/logP 0.23) but 3e-6 once saturated, so a fully collapsed softmax is an absorbing state, the same shape as the collapsed projection head. Caught on the way down, never recovered from; watch `Contrastive/gap_pool_entropy_L*`. **`eval/gap_pool_profile.py`** answers this from a checkpoint with no training run, and separates four causes of a flat reading (delocalised / channels localised but disagreeing / variance anti-aligned with content / variance simply the wrong statistic, tested against per-position factor recovery on held-out subjects). Its verdict is a **permutation test**, not a threshold: any concentrated weighting raises pooled variance just by averaging fewer things, so the null reassigns the same weights to random positions. An entropy threshold was tried first and **rejected true positives** — at P=256 with a factor planted in 4 positions, entropy reads 0.999 of 1.0 while cross-view correlation still goes 0.031 → 0.170. Regression-pinned in `scripts/smoke_bt_gap_pool.py` (12 sections). Ablations in `experiments/gap_pool_{variance,learned}.yaml`, which mirror `synthetic_causal.yaml` line-for-line because `launch.py` deliberately does not chain `_base_`. |
