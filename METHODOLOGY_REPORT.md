...
| Date | Changes |
|------|--------|
| 16 Sep 2026 | **Cross-modal reconstruction as an explicit separation pressure** (`--scale-cross-recon-loss`, `--cross-recon-start-step`; 0 by default, so every existing run is bit-identical). Plain reconstruction only ever shows the decoder a MATCHED content/style pair, so a content code that quietly encodes its own modality costs nothing — the separation we want is, under that objective, merely one of many optima. Swapping the pair prices it: the final decoder is run a second time on `(content_v0, style_v1)` and `(content_v1, style_v0)` and scored (masked L1, `training.losses.cross_reconstruction_loss`) against the view-SWAPPED batch, so content that carries modality misses its target by the full between-modality contrast difference, and style — the only input that differs between the two halves — has to carry modality for one content code to reach two targets. It is the converse of `--style-dropout-prob`, which pushes anatomy INTO content by decoding without style; the two are complementary. **Cost is one extra decoder-0 forward**, not a second model pass: `decoder_in` is built only from `code_q` tensors, so the decoder is a pure function of (content codes, style), and the coarser decoders reach the image ONLY through codebook conditioning at the next-finer level. That also fixes the scope — the content codes are deliberately held at their unswapped values, because a swap that was allowed to change the content code would stop measuring whether content ALONE is modality-invariant — and it is why the term requires level 0 in `--content-style-levels` (rejected in `update_args`, not at the first forward) along with `--inject-style-to-decoder` and two views. Style dropout is NOT applied to the cross decode: zeroing style there asks content alone to render the other modality, i.e. trains the exact leak the term exists to punish. **The degenerate solution is not closed by this term** — content can carry BOTH modalities' appearance and use style as a selector — so it is meant to be run with style kept low-capacity (`--style-spatial-size`, or a small `--style-nb-entries` under `--quantize-style`); `experiments/ablation_cross_recon.yaml` does that, and names its own control (the same file at `scale_cross_recon_loss: 0.0`, i.e. baseline + style injection, so the cross term is the only difference). Logged as `CrossRecon/mae` plus a per-direction pair: "T1 anatomy rendered as T2" and its reverse fail independently, and one collapsing while the mean looks mediocre is the failure mode the aggregate hides — the same asymmetry `--recon-view-balance` exists for. The perceptual term is deliberately omitted (the quantity of interest is the global tissue→intensity map, which is the pixel term's business, and LPIPS would double the cost of the only term that already decodes twice). Verified end-to-end on CPU in `tests/test_cross_reconstruction_loss.py`, which optimises the term on one paired volume whose two 'modalities' differ by an affine intensity map and then checks the model renders whichever modality the style says from one content code: the cross decode must land several times closer to the OTHER view than to its own. The same file pins the direction of the target (a decode that already equals the swapped batch costs zero; the unswapped batch does not), that the mask is swapped alongside it, and that a swap between IDENTICAL views is a no-op. |
| 9 Sep 2026 | **The causal panel's readout is a confound and an optimism, and both are now controllable** (`run_causal_recovery --readout-dim`, `--holdout-readout`; both opt-in, defaults byte-identical). PC never sees a representation — it sees `n_content` supervised `RidgeCV` reconstructions of the true factors, taken from a PCA basis. *(1) Width.* That basis is `min(64, max(n_content, N/5))` capped at the block's own width, so scoring a 48-channel encoder block against an 18432-dim embedding silently compared readouts of 48 and 64 dims; measured on the clean-content run, that asymmetry sits underneath a skeleton-F1 difference of 0.621 vs 0.813. `--readout-dim` pins it so two models can be read at equal readout capacity. *(2) Optimism.* The default readout decodes the same rows it was fit on, with the true labels — which is why the panel is documented as a diagnostic rather than as causal discovery. `--holdout-readout` fits on the 70/30 train split and runs PC on the held-out rows only, sharing the split with the existing `PC R²` diagnostic so that column becomes exactly the decoding quality of the columns PC is handed. A single split rather than cross-fitting: a cross-fitted row's decoding depends on every other row's label, and Fisher-Z assumes independent rows. It costs 70% of the rows, so PC below `20 * n_content` rows warns and below 20 errors. **Neither flag closes the deeper gap**, which is that the readout is supervised at all: the labels undo the representation's gauge before PC ever runs, so the panel measures whether *decoded* factors retain the SCM's conditional independencies, not whether the representation exposes the factors as coordinates. Blind discovery needs component-wise, not block, identifiability. Also noted while specifying that work: the generator is a nonlinear additive-noise model with **Gaussian** disturbances (`leaky_relu(w·z_pa) + 0.4·ε`), so PC with Fisher-Z is misspecified twice — a linear-Gaussian independence test on nonlinear mechanisms, and a CPDAG-only estimator — which is the real reason the orientation ceiling reads SHD 17 with 8 reversals *on the true factors*. Nonlinear ANMs are fully DAG-identifiable, so that class (not PC, and not LiNGAM, which additionally needs non-Gaussian disturbances and linear mechanisms) is the right estimator for this SCM. |
| 8 Sep 2026 | **A pretrained-DINOv3 reference point on this generator, scored by the existing protocols** (`eval/dinov3_embed_synthetic.py` → `eval/dinov3_identifiability.py`). The question is what a general-purpose 2-D foundation model already recovers from these volumes, both per factor and as a causal skeleton, so a VQ-VAE number has something external to be read against. The extraction script slices each volume into evenly spaced planes per axis, embeds them with a Hub checkpoint and saves embeddings, ground-truth latents and the SCM adjacency to one `.npz`; the scoring script reuses `identifiability_metrics` (per-factor CV R² with a permutation null, block-MCC, batched as in `run_dci_compare._score_block`) and `run_causal_recovery.evaluate_arrays` (parent-residualised partial R², PC skeleton recovery) rather than re-deriving either rule. Three decisions carry the measurement. *(1) Windowing.* The standard per-slice 1–99 percentile normalisation maps every plane onto the same range, which is the affine map style applies (`lut = base*gain + bias`) — the same erasure `--synthetic-normalize per_sample` performs, so the default estimates ONE window from a pilot of volumes and per-sample intensity survives into the ViT input. *(2) Token layout.* DINOv3 emits four register tokens after CLS; mean-pooling `last_hidden_state[:, 1:]` would average them in with the patches, so the patch prefix is inferred from the sequence length (`seq_len − (S/patch)²`) and the config value is only cross-checked against it. *(3) Baselines.* An absolute R² here is mostly a statement about the architecture — the untrained-encoder measurement in the 25 Aug entry applies unchanged — so the report carries a `--random-init` floor of the same architecture at a fixed seed and a downsampled-voxel baseline in every table, and the graph panel carries a `truth` row (PC on the ground-truth factors), which is the ceiling that separates "the embedding lost the graph" from "PC cannot recover it at this N". Alongside it, `run_causal_recovery --orientation` (opt-in, outputs otherwise byte-identical) scores edge DIRECTIONS against the true DAG's **CPDAG** via causal-learn's `dag2cpdag`, not against the DAG: PC identifies a Markov equivalence class, and the default `chain` SCM's class is entirely undirected, so a DAG comparison would charge every estimate for two edges that no observational method can orient. |
| 1 Sep 2026 | **Site-sharing audit (`probe/site_sharing.py`): are the decoding mechanism and the channel labelling the same at every latent position?** The field-valued SCM's identifiability results rest on A1 (site-shared mechanism, `f_j` independent of `u`) and A4 (site-shared decoding), and A1 is what lifts a *per-position* result to a statement about the *field* — one global element of the residual gauge group instead of an independent one at each `u`. Both are properties of a trained decoder, not of its architecture, and the formulation names the test: the homogeneity of decoder-Jacobian spectra across Λ bounds the region over which the identifiability results may be claimed. Two statistics, both from one block Jacobian `J_u = ∂x_k|B(u)/∂z(u)` taken by exact JVPs with the style code held fixed (so it is the derivative w.r.t. `z` alone, matching `x_k = g_k(z, s_k, γ)`). **(1) Spectral homogeneity.** The log singular spectrum splits into `scale = mean(log s)` and `shape = log s − scale`. Scale varies with local anatomy and is *expected* to — a background block decodes to almost nothing — so an undecomposed spectral map is a brightness map and nothing more; **shape** is the site-sharing test, since conditioning belongs to the weights rather than to where they are applied. **(2) Binding.** Hungarian-match `J_u`'s columns to a reference site's on `|cos|` (absolute, because a sign flip is one of the coordinate-wise bijections the gauge admits). Under A1 the answer is the *identity* permutation, because channel `j` is produced by the same filters everywhere. This is the empirical counterpart of the cross-position binding claim, and it is precisely what a probe on flattened patch features cannot see: flattening hands an unconstrained readout enough freedom to absorb a position-dependent permutation before scoring, so patch-MCC passes the failure mode A1 exists to exclude. **The two are orthogonal**, which the tests pin down: a per-position permutation collapses binding (1.000 → 0.092 on corrupted sites) while leaving the spectrum flat, and a conditioning drift does the reverse (ratio → 265, binding 1.000) — neither statistic substitutes for the other. **No invented thresholds.** Each site is measured across several subjects, giving the measurement's own noise floor, and the across-site spread is reported as a multiple of it; the floor is matched to the √n the subject-mean already bought (with a Bessel correction on σ), so the null sits at ratio ≈ 1 rather than at an arbitrary small number — a mismatch here would have made every model look homogeneous. **`frozen_norm` is a hard gate, not a footnote.** A purely convolutional decoder scores 1.0 on both with nothing learned, since the weights are shared by construction; what varies across `u` is *state* — nonlinearities and, decisively here, GroupNorm's whole-volume statistics. So the measurement reads the *effective, state-dependent* mechanism, and pinning the norm statistics must return ~1.0 or the instrument is broken and the run's numbers do not count. Output is a per-site validity map over Λ, which is what downstream identifiability metrics should be restricted to — replacing the foreground/background threshold with a region the theory justifies. Synthetic-only for now, and it refuses an ADNI run rather than rendering synthetic volumes for a decoder never operated at that point; nothing in either statistic needs ground-truth factors, so the real-data path is a loader swap. Read `probe/jacobian_spread.py` first: if its guard scan fails, ρ is unbounded and the blocks already overlap. Statistics are pure numpy with torch imported lazily, so `tests/test_site_sharing.py` exercises them against synthetic Jacobians with known answers — including block tiling at non-divisible latent/output ratios — with no torch build. |
| 27 Aug 2026 | **W&B run resumption.** An interrupted job (SLURM time limit, RunAI eviction, OOM) resumed its checkpoint correctly but opened a *second* W&B run, so a single training curve arrived split across two or more dashboard entries and `--wandb-group` aggregates double-counted it. The run id is now persisted to `<save_dir>/wandb_run_id.txt` — written before `wandb.init`, so a job that dies in its first seconds still leaves one behind — and `wandb.init(id=..., resume="allow")` appends the relaunch to the original run. The stored id is reused only when the process is continuing an existing `save_dir` (`--resume-training` or `--evaluate`); training restarted from step 1 gets a fresh id, since replaying steps W&B already holds would have them dropped and leave the run looking frozen. The same drop rule means the checkpoint-to-interruption interval keeps its pre-crash values rather than being overwritten, which is what keeps the curve continuous. `--wandb-run-id` forces an id, for reattaching to a run whose id file was lost. Sweeps are unaffected: when a sweep agent has already opened a run for the process, training logs into that run instead of initialising its own. |
| 25 Aug 2026 | **DCI was being scored on a PCA basis, which the metric cannot support; plus a readable single-run report.** *(1) The basis defect.* `score_reprs` applied `_reduce_reprs` (the `--probe-dim` PCA) BEFORE `_score_one_encoder`, so every DCI scope was scored on principal components. D asks "is this CODE dedicated to few factors" and C normalises by the code count — both are statements about one specific basis, and PCA replaces every code with a variance-ordered mixture of channels, so `dci_*` described the PCA basis rather than the encoder. Worse, `auto` reduces only the p≫n blocks: at N=2000 that put `dci_patch` on 64 components while `dci`/`dci_content` stayed on raw channels, so the three rows printed side by side were in different bases and not comparable. DCI now reads the unreduced blocks (`dci_reprs`), capped by `--dci-max-codes` (default 4096) using highest-variance **selection**, never a rotation, so each retained code stays a real `(channel, patch)` feature. The width used is emitted as `dci_n_codes*` in the table and CSV, and the table warns when two scopes differ. Probes and `_effective_rank` are unchanged. *(2) What patch-pooled DCI can still support.* Even in the right basis, patch pooling turns one channel into ~512 near-duplicate cells, which dilutes C and inflates D by construction (a factor on one channel reads C ≈ 1 − log 512/log 22528 ≈ 0.38 at 8³ vs ≈ 1.0 at gap, same information). The permutation null holds X fixed so it absorbs part of this, but what survives is monotone-only: compare a scope against the SAME scope at equal `n_codes`, never one scope against another or one pooling against another. *(3) `eval/identifiability_report.py`.* One page: per-factor R² gap at each factor's assigned pooling, the MCC ladder gap→stats→patch, DCI per scope, each against an untrained twin scored through the identical path (`--floor` defaults ON here, unlike `run_dci_compare` whose default stays off for CSV comparability). The verdict block refuses to report a learned delta inside `NOISE_FLOOR = 0.05` — calibrated from `patch_mcc_decay --calibrate`, where the smallest perturbation that reliably moved block-MCC was +0.0447 and a real run's whole training excursion was 0.04. Every metric rule is imported from `run_dci_compare`, not re-derived. `--self-test` scores planted numpy data with no torch. *(4) `mcc_by_pooling`* extracted from `score_encoder_live` so the in-training ladder and the offline report cannot drift. *(5) Report readability:* a `HOW TO READ THIS REPORT` preamble (gap not absolute; floor delta; never difference across axes; MCC is not invariant to the invertible maps block-identifiability permits — an exactly invertible re-gauge costs up to 0.38) and numbered section dividers. **Open:** claim #1 in `paper/EVIDENCE.md` ("completeness tied at patch pooling", block-MCC 0.540/0.535) is a NULL result, and its power was never established — `patch_mcc_decay --calibrate` was run at MCC ≈ 0.84, not at the ≈ 0.54 operating point where the claim sits. Re-run it there before the tie carries a paper. |
| 20 Aug 2026 | **Contrastive learning REALLOCATES content rather than adding or losing it — plus three evaluator defects that hid this for the length of an investigation.** Started from an apparent identifiability degradation at `vqvae_scaling_rates [4]` vs `[2]`; ended somewhere else entirely. **Three defects in the evaluators, all fixed.** *(1) Train/eval factor-distribution mismatch.* `--causal` defaulted to i.i.d.-with-a-warning while runs trained under an SCM. Measured at **identical `gap` pooling**, `ventricle_size` reads **0.695** under `--causal match` without a null and **0.001** as a null-subtracted GAP under `iid` — because `brain_size` is recovered at 0.92 and correlates ~0.8 with it under the random graph, so the probe was reading brain_size in disguise. Now defaults to `match` in `content_rank_pca` / `run_dci_compare`, and `--causal-eval` is ON via `BooleanOptionalAction` in `run_dci_synthetic`. **Caveat that matters more than the default: `match` is for aggregate ranking, `iid` is for per-factor attribution.** `match` inflates every factor with a recoverable parent; any per-factor claim must be read off `iid`. *(2) p≫n probe overfit.* `--probe-dim` defaulted to 0, leaving the patch block at 22528 features against N=2000, where ridge returns *negative* R² on weak targets. Now `auto`: reduces only blocks with `d > N/4` to `min(64, N/4)`, leaving `gap` (44) and `stats` (176) at full width so well-conditioned readouts are not silently reshaped; an explicit integer still reduces every block, for equalising a channel-count confound. *(3) `--random-init` never ran on a GPU.* `load_model_from_run_dir` returned before its `model.to(device)`, so the untrained zero point — the number every `block_mcc` should be reported as a gap over — had been unobtainable for the life of the script. **The two floors, now measured** (untrained encoder, this architecture, `gap`, N=400): `content→content/block_mcc` **0.705**; per-factor test R² `brain_size` **0.808**, `ventricle_size` **0.513**, `cortical_thickness` 0.582, `lesion_y` 0.262 — a random projection reads these off the channel mean. And `content→view/acc` **0.948 untrained**, so a trained model's `c>view = 1.000` sits ~0.05 above a random projection: at `gap` pooling that metric is near-saturated by construction (T1/FLAIR differ globally in intensity statistics) and is NOT evidence of a view leak the objective introduced. **The finding.** Contrastive vs its own recon-only baseline, both `vqvae_model.pt` @ step 22001, 765,991 params, `--causal iid --probe-dim 64 --poolings gap,8x8x8`: `ventricle_size` **−0.012 / 0.596**, `lesion_x/y/z` **−0.015/−0.020/−0.024 / 0.119/0.153/0.104**, `temporal_atrophy` **0.800 / 0.281**, `lr_asymmetry` **0.879 / 0.463**, `cortical_thickness` 0.875 / 0.789, `sulcal_widening` 0.179 / 0.042, `brain_size` 0.922 / 0.894 — **mean 0.398 / 0.382**. Total factor information is unchanged; what moves is *where it lives*: `gap` eff_rank **4.2 → 30.9** (the baseline stores content in the spatial layout, the contrastive model in channel identity), `gap` R² 0.337 → 0.392 while patch R² 0.575 → 0.475 (the baseline beats the 16³ raw-pixel ceiling of 0.550 at patch; the contrastive model does not). Gains are all boundary-displacing geometry, losses are exactly the two intensity-contrast factors. **`temporal_atrophy` is a localized Gaussian bump and it more than doubled**, so the split is not local-vs-global but **geometric vs intensity-contrast**. Same pattern at scaling 4 against its own matched baseline (`ventricle_size` 0.530 → 0.001, lesions 0.22–0.35 → ~0.00), so this is a main effect of the objective, not an interaction with the scaling rate — and the scaling-rate degradation that opened the investigation does not survive matched checkpoints at all. **Candidate mechanism, UNCONFIRMED.** The modality LUT (`eval/synthetic_dataset.py:447-452`): the lesion is 0.40 against WM 0.80 in T1 and 1.00 against WM 0.40 in FLAIR — its contrast **reverses sign** between views; the ventricle (CSF 0.10) against WM goes 0.70 → 0.30, same sign, 2.3× amplitude. A view-shared code cannot carry sign-inverting evidence without a rectifying detector, and nothing in Barlow Twins' correlation structure asks for one; reconstruction has no such constraint since each view is decoded from its own code. This predicts the exact split and the severity ordering (sign flip → ~0.00, amplitude change → −0.012). **Open:** confirm with per-modality render Jacobians in `eval/generator_defects.py`. If it holds, `lesion_*` and `ventricle_size` are mislabelled as pure content — their image evidence is partly style, and the benchmark is penalising the model for correctly rejecting view-varying evidence. **Housekeeping:** `build_synthetic_test_set(causal=None)` survives for direct callers with a warning that now states the measured consequence; ~19 diagnostic scripts still call it without passing `causal` and should be swept deliberately. |
| 17 Aug 2026 | **Generator-side identifiability audit (`eval/generator_defects.py`).** Five defects in the synthetic data-generating process, each measured rather than argued; all are checkpoint-free, so they bound what *any* encoder can recover and should be ruled out before a low per-factor R² is read as a model result. **(1) Squash ceiling.** `render_structure._sq` clamps content to [-1,1] by default; z~N(0,1) saturates 31.6% of draws, capping recovery at R² **0.906 linear / 0.940 nonlinear**. `--synthetic-clean-content` switches it to tanh, which is injective (nonlinear ceiling 1.000) but whose *linear* ceiling is only **0.933** — and `cv_probe_r2` / `block_mcc` default to `kind="ridge"`, so the flagship clean-content runs still carry a ~0.93 headline ceiling. Two new decoupled knobs: `--synthetic-content-squash {auto,clamp,tanh,none}` (auto = the historical coupling to `clean_content`) and `--synthetic-content-prior {normal,uniform}`, where uniform puts every factor in [-1,1], makes the squash the identity, and removes the ceiling for every probe class (verified: 0.0% saturation, ceiling 1.000). Under `--synthetic-causal` / `--synthetic-hierarchical-content` the uniform prior is applied as a per-dim probability-integral transform (`2Φ((z−μ)/σ)−1`, pilot-estimated), so it is monotone per factor and leaves the DAG's dependence structure intact. **(2) Amplitude vs noise floor.** Per-dim render sensitivity against the *measured* rendering-noise floor (Rician + bias field), reported per-voxel and as a matched filter. Boundary displacement in voxels is **not** the predictor of recoverability — effect × boundary *area* is: `cortical_thickness` (0.96 vox) reaches matched-filter SNR 93 because it moves the whole outer GM surface, while `ventricle_size` (0.80 vox) sits at **2.9** and `lesion_x/y/z` at **2.2–2.7**, i.e. at or below the floor, under the flagship config. `--synthetic-identifiable-ventricle` lifts the ventricle to 6.8 (better than a naive ×2 amplitude, which drops the low end onto the |x|>0.05 septum split); new `--synthetic-lesion-radius` (WM margin now tracks it) lifts the lesion dims to ~3.8 at 0.18, and to **4.3–4.7** combined with the uniform prior. New `--synthetic-content-amp-scale` gives per-dim effect-size control (vs `--synthetic-content-scale`, which moves all dims together). **(3) `per_sample` normalization erases style.** `lut = base*gain + bias` is affine in intensity and per-sample foreground z-scoring removes exactly an affine map: a style swap at fixed anatomy leaves a residual of **0.084** of the volume's own contrast under `per_sample`/`shared` vs **0.759** under `fixed_reference` (matched-filter SNR: gain 16.8→1.4, bias 18.8→2.9). `SyntheticBrainDataset` now warns when a run combines `per_sample`/`shared` with `n_style > 0`. **(4) Inert lesion dims.** In `lesion_mode="field"`, `z_content[2:5]` are provably dead — sign sweeps give `max|dx| == 0.00000`, 0 voxels changed — so a full-`z_content` block-MCC silently averages in 3 unrecoverable columns; the dataset now warns. They are live in `sphere` mode (8593 voxels changed). **(5) brain_size / cortical_thickness degeneracy — confirmed.** `radii_gm = radii_wm + thickness` makes both act identically on the outer GM boundary; their render-Jacobian cosine is **+0.61** (defaults) / **+0.39** (flagship) and they hold the two lowest unique-variance shares (0.57, 0.60). This is mutual confusability, not individual weakness — both have high individual SNR — so a per-dim breakdown showing exactly this pattern is a rendering artifact, not a model failure. A second, weaker cluster: `temporal_atrophy` vs `ventricle_size`/`brain_size` (cos −0.36 / −0.39). All defaults are unchanged and byte-identical (asserted for both `clean_content` settings). **Degeneracy closed.** The dim0/dim5 coupling is *not* fixable by re-parameterizing the radii: both Jacobians live in the 2-D space spanned by the two concentric shells, so their cosine is fixed by the shells' relative energy — and that energy is the opposite of what geometry suggests, because under `fixed_reference` the global mean (0.552) lands on the GM intensity (0.5), so after centring the outer GM/background edge nearly vanishes and the WM/GM edge dominates ~20:1. Measured cos(z0,z5) across the radial family: `additive` **+0.43** (best — the dominant inner edge is z0-only), `midsurface` **−0.49**, `nested` **−0.92** (worst — puts both factors on the dominant edge). The fix has to leave the radial subspace, which is exactly the property that already makes `lr_asymmetry` (an l=1 x-mode) and `sulcal_widening` (a high-frequency mode) the two most uniquely-identified dims. New `--synthetic-cortex-parameterization {additive,nested,midsurface,patterned}`: **`patterned`** modulates thickness by the l=2 zonal harmonic along A–P, zero-mean and orthogonal to both the l=0 monopole (brain_size) and the l=1 x-dipole (lr_asymmetry). New `--synthetic-center-local-deformations` applies the same reasoning to `temporal_atrophy`, whose strictly-positive Gaussian bump carries a DC component that *is* a brain_size offset — the identical confound `sample_gp_field` already centres its fields to avoid. Combined, at the flagship's real `synthetic_res: 64`: cos(z0,z5) **+0.345 → +0.090**, cos(z0,z6) **−0.39 → +0.17**, and unique-variance shares brain_size **0.768 → 0.965**, cortical_thickness **0.871 → 0.988**, temporal_atrophy **0.783 → 0.980**; the weakest dim in the whole vector is then 0.946. Two caveats: `patterned` REDEFINES z5 as regional (front/back) cortical thinning at unchanged mean thickness — a different factor, so its per-factor R² is not comparable against an `additive` run — and it costs cortical_thickness some raw SNR (457 → 142, still far above the floor). Note also that most of the *amplitude* problem is a res=32 artefact: at res=64 no dim is below the matched-filter floor even before the fixes (weakest 6.1), so at the training resolution the ceiling and the degeneracy are the substantive wins. `experiments/synthetic_identifiable.yaml` is `synthetic_causal.yaml` with all of the above applied and the model/objective half held verbatim, for a clean A/B. |
| 16 Jun 2026 | **Per-encoder evaluation metrics:** `eval/cross_reconstruction.py` now reports per-view (T1 vs T2) diagnostics alongside the existing pooled separation metrics, so each encoder's health is visible during in-training evaluation rather than only in aggregate. For every masked level it logs: **effective rank** of each view's content and style features (`content/eff_rank_v0`, `style/eff_rank_v1`, …) — the Roy–Vetterli `exp(entropy of normalized singular values)`, matching `eval/run_dci_compare._effective_rank` so the figure is comparable across the synthetic and ADNI paths; a value far below the nominal channel width flags a (partly) collapsed encoder. **Active-channel fraction** (`*/active_frac_v{0,1}`, channels with std > 1% of the max) and **mean activation scale** (`*/feat_std_v{0,1}`) catch dead/saturated channels and shrinking response. **Encoder-balance ratios** (`content/view_balance_eff_rank`, `…_feat_std`, style variants) report min/max of the two views' stat — ~1.0 when both encoders are equally exercised, well below when one view lags. A **per-view anatomy probe** (`content/diagnosis_probe_acc_v{0,1}`, `…/diagnosis_info_v{0,1}`) catches a view whose content has lost diagnostic signal even when the pooled probe stays high. A **content alignment gap** (`content/alignment_gap` = true-pair cross-view cosine − shuffled-pair cosine, with `content/cross_view_cosine_shuffled`) isolates genuine cross-view pairing from the anisotropy that inflates raw cosine. All keys are emitted per level (`_L{lvl}`) with the finest level mirrored unsuffixed, and require no training-loop changes — the periodic (every 2000 steps) and end-of-training separation passes already forward every `cs_metrics` key to TensorBoard/W&B/CSV. Metrics apply whether the two views share an encoder or use `--separate-encoders`. **Per-encoder synthetic informativeness:** `eval/dci.py` `compute_dci_synthetic` gained a `per_encoder` flag (default off). When set with two views present, it scores the **second** encoder's representations against the GT latent factors too — the `content_v2→content`, `content_v2→style`, `style_v2→style`, `style_v2→content` blocks — alongside the existing primary-view blocks, so per-view DCI/informativeness (GBT `informativeness_test`, linear RidgeCV `informativeness_ridge`, per-factor test-R², and `block_mcc`) is visible for both encoders. Previously the second view's features (`content_v2`/`style_v2`) were extracted but used only for the content→view leakage probe, never scored for factor informativeness. View-2 style is read against the view-2 style factors (`z_style_v2`); content factors are shared. The periodic (`--dci-every`, default 2000 on synthetic) and end-of-training (`--eval-dci`) DCI passes pass `per_encoder=--separate-encoders`, so the `_v2` blocks flow to TensorBoard/W&B (`dci_synthetic/content_v2->content/…`) and the `dci_synthetic.csv` automatically only when each modality has its own encoder. |
| 3 Jun 2026 | **Spatial style bottleneck:** `--style-spatial-size N` (default `0` = off) average-pools each injected style tensor to an `(N, N, N)` grid — clamped per-axis to the current size, so it only ever downsamples and is a no-op at coarse levels — at extraction time in `models/vqvae.py` (`VQVAE._bottleneck_style`), **upstream** of both the optional style codebook and the decoder FiLM/concat injection. This caps the *spatial* capacity of the style pathway so it carries the global tissue→intensity contrast transform rather than anatomy (which is left to the discrete content code); the decoder's existing trilinear FiLM/concat upsampling restores feature resolution. Rationale: a full-resolution style tensor (≈46³ at level 0) has anatomical spatial extent and lets reconstruction route structure through style — `quantize_style` caps bits-per-location but not the spatial grid, so it does not close this leak; a coarse grid does. The pool is parameter-free (checkpoint-compatible) and scoped to the reconstruction/quantization style path only — the style-contrastive features (`_style_hz_*`, pooled from encoder outputs in `training/main_multimodal.py`) are unchanged. Ablate against the legacy full-resolution behaviour by leaving the flag at `0`. |
| 23 Apr 2026 | **Jukebox perceptual reconstruction loss:** `--recon-loss-fn {BaselineLoss, JukeboxPerceptualLoss}` (default `BaselineLoss`) selects the reconstruction objective. `JukeboxPerceptualLoss` (`training/losses.py`) combines (a) a spectral term — FFT-magnitude MSE on the 3D volume, (b) a perceptual term — LPIPS on randomly-sampled axial/coronal/sagittal 2D slices (2.5D), and (c) a pixel term whose distance is selectable via `--jukebox-pixel-loss-type {mse, l1}` (default `mse`). Predictions are clamped to `[-1, 1]` before FFT/LPIPS to prevent overflow, and the pixel/FFT terms are masked by the brain mask when present so background voxels are not penalized. **PatchGAN discriminator:** `--use-gan` adds a 3D PatchGAN (`disc-base-channels` default 32, `disc-lr` default 4e-4) trained jointly with the VQ-VAE. The generator term `-mean(D(recon)) * scale_adv_loss` (default `--scale-adv-loss 0.1`) is gated by `--gan-start-step` (default 0) so reconstruction can stabilise before the adversarial signal kicks in. **Style InfoNCE loss:** `style_infonce_loss` (`training/losses.py:958`) is a within-modality supervised contrastive objective on the pooled style vectors — all same-view / different-subject pairs are positives, cross-view pairs are hard negatives (τ=0.1). Enabled by `--scale-style-contrastive-loss` (default 0.0, disabled); intended to collapse style representations onto the modality manifold more aggressively than the plain cross-view InfoNCE. The denominator uses `float("-inf")` masking (rather than `finfo.min`) to guarantee exact zeroing of the self-similarity term after softmax. **Separation-score overhaul:** `eval/cross_reconstruction.py` now computes a per-level `separation_score = 0.5 · (content_modality_invariance + style_subject_invariance)`, where `content_modality_invariance = 1 − |content→modality acc − 0.5|·2` (content probe should sit at chance for modality) and `style_subject_invariance = max(0, 1 − (style→subject top-1 retrieval − chance)/(1 − chance))` (style should not identify subjects). Per-level and aggregate (mean/min/max) scores are logged to TensorBoard and W&B each validation, and the **best-checkpoint selector** now tracks highest `separation_score` (not lowest loss); `best_separation_score` is restored on resume. **L2 normalisation before probes:** all linear/MLP probes in cross-reconstruction evaluation now fit in L2-normalized feature space (`Normalizer(norm='l2')` pipeline step), removing scale-dependent artifacts and stabilising probe accuracy across runs. **Per-level codebook sizing + level skipping:** `--vqvae-nb-entries` now accepts either a single int (broadcast to all levels) or one int per level (`--vqvae-nb-entries 256 384 512`), supporting asymmetric codebook pyramids; combined with the existing content-ratio pyramid this enables coarser levels to use smaller codebooks. **EMA update hygiene:** the codebook EMA `update` step was revised (commit `c241a72`) so `cluster_size` and `embed_avg` use matched-decay running averages and dead-entry detection reads the post-update statistics, reducing false-positive resets in the first few hundred steps. **Checkpoint & resume robustness:** saved checkpoints now include (i) MoCo queue state for every level, (ii) `best_metric_name`/`best_metric_value` for whichever selector is active, (iii) discriminator + discriminator-optimizer state when `--use-gan`, and (iv) full LR-scheduler state. On resume, partial state loading tolerates shape mismatches on mask buffers (`47d06d5`) so runs started under one mask mode can be continued under another without re-initialisation errors. **New L0-content-width sweep:** `scripts/sweep_content_L0.sh` (Phase 2 of the channel-budget study) fixes `hidden_channels=32`, `L1/L2` content at 10 channels (≈0.3125 ratio), and sweeps absolute `L0` content width over `{4, 8, 13, 16, 20}`. Selection rule: smallest `C_L0` where the anatomy probe plateaus AND the modality probe stays at chance. Sweep uses `--mask-mode fixed`, `--cross-view-negs-only`, `--patch-contrastive`, `--scale-style-contrastive-loss 10`. `scripts/sweep_config.yaml` was also updated for the new Bayesian objective (now `separation_score_mean`). **Evaluation notebook:** `eval/view_latents.ipynb` gained per-level style-codebook analysis (utilisation histograms, dead-entry counts, CN-vs-AD code-usage comparison) and a separation-score trend panel; plus an `anatomy_probe.py` standalone script for external probe evaluation. **L2 / L1 reconstruction metrics:** `eval/evaluation.py` now reports raw L1 and L2 voxel-wise reconstruction error alongside PSNR/SSIM in the validation summary. |
