...
| Date | Changes |
|------|--------|
| 19 Aug 2026 | **Two eval defaults changed — they were jointly manufacturing a phantom result.** A scaling-rate comparison (`vqvae_scaling_rates [4]` vs `[2]`, single level, everything else held) appeared to show a large identifiability degradation at scaling 4, concentrated in the localized factors. It was almost entirely an artifact of two `content_rank_pca` / `run_dci_compare` defaults, and re-running the *same two checkpoints* with both corrected collapsed the mean per-factor R² gap from **0.209 → 0.032**. **(1) Train/eval factor-distribution mismatch.** `--causal` defaulted to i.i.d.-with-a-warning while the runs were trained under an SCM. This is not a wash across models: scoring an SCM-trained run on i.i.d. factors reads dependence-through-parents as lost information, and the penalty *grows* with how well a model fits the training distribution, so it mimics degradation. **(2) p≫n probe overfit.** `--probe-dim` defaulted to 0 (no reduction), leaving the patch block at 22528 features against N=2000, where the ridge returns *negative* R² on weak targets. Together they inverted the sign of the lesion dims (`lesion_x` s4 **−0.154 → 0.694**, s2 0.239 → 0.686 — i.e. equal, not degraded) and moved `ventricle_size` from "dead in both, generator defect" to **0.695 / 0.714**. New defaults: `--causal match` (`content_rank_pca`, `run_dci_compare`) and `--causal-eval` ON via `BooleanOptionalAction` (`run_dci_synthetic`, opt out with `--no-causal-eval`); `--probe-dim auto`, which reduces **only** blocks in the p≫n regime (`d > N/4`) to `min(64, N/4)` and leaves `gap` (44) and `stats` (176) at full width, so the well-conditioned readouts are not silently reshaped — an explicit integer still reduces every block, which is what you want when equalising a channel-count confound. The `build_synthetic_test_set(causal=None)` path survives for direct callers but its warning now states the measured consequence; ~19 diagnostic scripts still call it without passing `causal` and should be swept deliberately. **What actually survives the correction:** a 0.032 mean gap, 84% of it in `sulcal_widening` (0.510 vs 0.684) and `cortical_thickness` (0.805 vs 0.876), both read at `gap`; every other factor is within ±0.02 and `lesion_x` is marginally *better* at scaling 4. That residue is itself confounded — the two checkpoints compared were `vqvae_best.pt` @ step 38001 vs `vqvae_model.pt` @ step 22001, different selection criteria 16k steps apart, in the direction the documented patch-MCC decay predicts. Re-run at matched checkpoints before treating it as an architecture effect; if `sulcal_widening` survives it is a Nyquist argument specific to high-frequency factors (a 16³ latent represents ~8 cycles/volume before the global average), not an objective one. |
