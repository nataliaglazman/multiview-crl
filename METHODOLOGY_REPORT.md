...
| Date | Changes |
|------|--------|
| 8 Sep 2026 | **The identifiability report on the training curve (`--identifiability-every`), and why it never matched `selection/*`.** The in-training TensorBoard metrics and `eval.identifiability_report` were disagreeing for structural reasons, not because either was wrong: they are different estimators, of different quantities, on different data. `selection/*` scores the **val** split (generator seed 43) at `--selection-probe-dim 0`, on `--patch-grid`, over 2 seeds, with no floor, and its headline `overall_score` is a mean of four derived scores whose only content term is `_headroom(mcc_cc, mcc_cc_null)` — block-MCC at **stats** pooling, which is permutation-invariant over voxels and so cannot see `lesion_x/y/z` at all. The report scores the **frozen test** split (seed 44) at `--probe-dim auto`, on `8x8x8`, over 3 seeds, and its headline is **per-factor R² at each factor's `FACTOR_POOLING`-assigned rung, minus an untrained twin averaged over 3 seeded draws**. Every one of those axes moves the number, and two of them move it a lot: unreduced probes carry a p≫n negative bias (mean R² −0.27 on a *shuffled* target, ~−0.04 reduced), and at patch pooling an untrained encoder already reads R² > 0.8 on most factors and block-MCC ~0.86, so a raw number there is mostly architecture. Nothing in TensorBoard was on the report's axis, so no amount of squinting at `selection/overall_score` was going to predict the report — and the composite is additionally known to *rise* on a degraded representation (measured: separation 0.414 → 0.546 while effective rank fell 37.9 → 6.5), which is why the `info_all` completeness gate exists. **Fix:** `eval/identifiability_report.py` is split into `extract_reprs` (the only torch) + `score_extracted` (every metric rule), with `score_run` and the new `score_model_live` as its two doors, so a checkpoint on disk, its floor twin and a live encoder are scored by one implementation. `report_scalars` flattens the printed tables to TensorBoard tags — `identifiability/r2_learned/<factor>` *is* table 1's LEARNED column, with `r2_raw` and the constant `r2_floor` logged beside it so a learned dip caused by a rising floor is visible rather than inferred. Training gains `--identifiability-every N` (log-only; it cannot reach `separation_score`, the selection gate or the best-checkpoint choice, so past runs' "best" is unmoved), whose defaults *mirror the offline script's argparse defaults rather than the selection block's* — that mirroring is the feature. The floor is computed once at the first firing through `score_run(random_init=True)` and reused, since a floor redrawn per firing would inject its own sampling noise into every learned curve. Two hazards handled: the floor twins call `torch.manual_seed` on the **global** RNG this training loop draws augmentations, Gumbel noise and dropout from (snapshotted and restored, or turning on a log-only diagnostic would change the trajectory it measures), and the cached test set is ~8 GB resident at the defaults while training itself does not cache (`--identifiability-no-cache-testset`). |
