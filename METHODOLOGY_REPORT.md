...
| Date | Changes |
|------|--------|
| 20 Aug 2026 | **Contrastive learning REALLOCATES content rather than adding or losing it — plus three evaluator defects that hid this for the length of an investigation.** Started from an apparent identifiability degradation at `vqvae_scaling_rates [4]` vs `[2]`; ended somewhere else entirely. **Three defects in the evaluators, all fixed.** *(1) Train/eval factor-distribution mismatch.* `--causal` defaulted to i.i.d.-with-a-warning while runs trained under an SCM. Measured at **identical `gap` pooling**, `ventricle_size` reads **0.695** under `--causal match` without a null and **0.001** as a null-subtracted GAP under `iid` — because `brain_size` is recovered at 0.92 and correlates ~0.8 with it under the random graph, so the probe was reading brain_size in disguise. Now defaults to `match` in `content_rank_pca` / `run_dci_compare`, and `--causal-eval` is ON via `BooleanOptionalAction` in `run_dci_synthetic`. **Caveat that matters more than the default: `match` is for aggregate ranking, `iid` is for per-factor attribution.** `match` inflates every factor with a recoverable parent; any per-factor claim must be read off `iid`. *(2) p≫n probe overfit.* `--probe-dim` defaulted to 0, leaving the patch block at 22528 features against N=2000, where ridge returns *negative* R² on weak targets. Now `auto`: reduces only blocks with `d > N/4` to `min(64, N/4)`, leaving `gap` (44) and `stats` (176) at full width so well-conditioned readouts are not silently reshaped; an explicit integer still reduces every block, for equalising a channel-count confound. *(3) `--random-init` never ran on a GPU.* `load_model_from_run_dir` returned before its `model.to(device)`, so the untrained zero point — the number every `block_mcc` should be reported as a gap over — had been unobtainable for the life of the script. **The two floors, now measured** (untrained encoder, this architecture, `gap`, N=400): `content→content/block_mcc` **0.705**; per-factor test R² `brain_size` **0.808**, `ventricle_size` **0.513**, `cortical_thickness` 0.582, `lesion_y` 0.262 — a random projection reads these off the channel mean. And `content→view/acc` **0.948 untrained**, so a trained model's `c>view = 1.000` sits ~0.05 above a random projection: at `gap` pooling that metric is near-saturated by construction (T1/FLAIR differ globally in intensity statistics) and is NOT evidence of a view leak the objective introduced. **The finding.** Contrastive vs its own recon-only baseline, both `vqvae_model.pt` @ step 22001, 765,991 params, `--causal iid --probe-dim 64 --poolings gap,8x8x8`: `ventricle_size` **−0.012 / 0.596**, `lesion_x/y/z` **−0.015/−0.020/−0.024 / 0.119/0.153/0.104**, `temporal_atrophy` **0.800 / 0.281**, `lr_asymmetry` **0.879 / 0.463**, `cortical_thickness` 0.875 / 0.789, `sulcal_widening` 0.179 / 0.042, `brain_size` 0.922 / 0.894 — **mean 0.398 / 0.382**. Total factor information is unchanged; what moves is *where it lives*: `gap` eff_rank **4.2 → 30.9** (the baseline stores content in the spatial layout, the contrastive model in channel identity), `gap` R² 0.337 → 0.392 while patch R² 0.575 → 0.475 (the baseline beats the 16³ raw-pixel ceiling of 0.550 at patch; the contrastive model does not). Gains are all boundary-displacing geometry, losses are exactly the two intensity-contrast factors. **`temporal_atrophy` is a localized Gaussian bump and it more than doubled**, so the split is not local-vs-global but **geometric vs intensity-contrast**. Same pattern at scaling 4 against its own matched baseline (`ventricle_size` 0.530 → 0.001, lesions 0.22–0.35 → ~0.00), so this is a main effect of the objective, not an interaction with the scaling rate — and the scaling-rate degradation that opened the investigation does not survive matched checkpoints at all. **Mechanism: it is cross-view AMPLITUDE ASYMMETRY, not sign inversion** (`eval/view_consistency.py`, new — checkpoint-free, so it bounds what any view-invariance objective could retain). Resampling one latent dim with the rendering seed held fixed (so Rician noise and the bias field cancel) and comparing the two views' render deltas gives, per factor, `cos(Δx_T1, Δx_FLAIR)` and `‖Δx_FLAIR‖/‖Δx_T1‖`. The sign hypothesis — the lesion is 0.40 *below* its WM surround in T1 and 0.60 *above* it in FLAIR, so its contrast reverses — **is real but does not predict the outcome**: `ventricle_size` is the *most* view-consistent factor measured (cos **+0.992**) and is the one most completely destroyed (−0.608), while `brain_size` (+0.031) and `temporal_atrophy` (+0.018) are near zero and are kept and improved. Spearman(cos, model delta) = **+0.150**. The amplitude ratio orders it instead: Spearman(|log₂ ratio|, model delta) = **−0.700**, with clean separation and no overlap — every kept factor at |log₂ ratio| ≤ **0.386**, every destroyed one ≥ **1.214**, a 3× gap. So a cross-view objective discards the factors whose evidence arrives at very different strength in the two views (ventricle 2.3× weaker in FLAIR, lesion 2.7–4.1× stronger) and keeps those arriving at comparable strength. The measurement validates against the LUT to three digits: CSF−WM is −0.70 in T1 and −0.30 in FLAIR, predicting a ventricle ratio of 0.4286 — measured **0.431**. Two readings that follow: (a) the "geometric factors are view-consistent" intuition is false for this generator, because the WM/GM edge itself inverts (WM−GM is +0.30 in T1, −0.40 in FLAIR) while the outer GM/background edge does not (+0.50 vs +0.80), so a factor moving both boundaries cancels to a near-zero cosine with large spread — a mid cosine means *mixed* evidence, not weak; (b) statistically the three lesion dims are one factor group, so the honest count is 2 destroyed groups vs 5 kept and n=9 overstates the rank correlation — the clean separation is the stronger evidence. **Causal test still open:** set both LUTs' `lesion_int` to a fixed offset above their own WM (e.g. T1 1.10, FLAIR 0.70 — both +0.30) so the lesion's evidence becomes view-symmetric *and* same-signed, retrain, and see whether the contrastive arm retains lesion position. **The inflation is CREATED BY TRAINING — the single most transferable result here.** The same s2 contrastive checkpoint, scored two ways: at `gap` pooling on SCM-matched factors (`run_dci_synthetic`, N=400, GBT) it reads `lesion_x` **0.562**, `lesion_y` **0.753**, `ventricle_size` **0.651**, mean **0.677**; at `--causal iid` (8³, probe-dim 64, ridge) the same weights read **−0.015 / −0.020 / −0.012**. The causal numbers are *not* the floor — the architecture-matched untrained init (now runnable, see the `--random-init` fix) scores `lesion_x` 0.024, `lesion_y` 0.262, `ventricle_size` 0.513, mean 0.270, so training genuinely added **+0.538 / +0.490 / +0.138**. What makes it decisive is that this is **`gap` pooling**: a global average pool carries no spatial information by construction and cannot localize anything, so `lesion_x = 0.562` read off a channel mean cannot be lesion localization — it is the probe walking the SCM from factors the model *does* encode. The untrained arm cannot do the same walk because it encodes the parents badly (`lr_asymmetry` 0.092, `sulcal_widening` 0.220, against the trained model's 0.863 / 0.762). So the SCM bonus scales with how well a model fits the training distribution — the general form of the `--causal` help's own warning, now measured, except that here it looks like *success* on factors the model does not encode rather than like degradation. Three consequences: (a) any per-factor identifiability claim must be read at `--causal iid`; (b) a causal per-factor number must never be compared across models of differing quality, because the better model collects a larger free bonus; (c) the in-training `dci_synthetic/*` TB curves use the run's own causal val set and carry this inflation throughout — they are a distribution-fit measure, not an identifiability measure. **Two candidate explanations for `ventricle_size` specifically, both dead.** Render degeneracy: `generator_defects --tests degeneracy` puts the `brain_size`↔`ventricle_size` render-Jacobian cosine at **−0.01** with a unique-variance share of **0.928**, so the encoder is not visually confusing them; and uniqueness does not order the outcome anywhere (Spearman −0.133, kept 0.742–0.994 overlapping lost 0.918–0.968). Weak-signal: Spearman(baseline R², delta) = **+0.000** exactly — `sulcal_widening` has the least available signal of any factor (0.042) and is kept and improved 4×, while `ventricle_size` is the third strongest (0.596) and is destroyed. That retires "weak factors die first" (the ρ +0.72 reading in `patch_mcc_decay.py`) as a general account. Relative to available signal all four destroyed factors are destroyed *equally* — 102%, 113%, 113%, 123% of the baseline's recovery — so `ventricle_size`'s larger absolute delta is a larger baseline, not a harder hit. Two ventricle-specific generator defaults remain worth switching before drawing conclusions about it: `vent_dist = deformed_dist` (`eval/synthetic_dataset.py:381`) reads the boundary off the *deformed* radius, where the nuisance field is "~50% of the tiny ventricle radius", and `tissue_map[fissure_mask] = 1` (`:395`) gives the longitudinal fissure the *same CSF label and intensity* as the ventricle, so a `gap`-pooled "total CSF" readout tracks the brain-size-driven fissure sheet. `--synthetic-identifiable-ventricle` flips both and lifts SNR 2.9 → 6.8. **Also banked, and robust to the causal/iid choice:** content→style test R² **−0.226** and content→style block-MCC **0.128** against the untrained arm's 0.445 / 0.586 — the objective genuinely purges style from content. **Housekeeping:** `build_synthetic_test_set(causal=None)` survives for direct callers with a warning that now states the measured consequence; ~19 diagnostic scripts still call it without passing `causal` and should be swept deliberately. |
