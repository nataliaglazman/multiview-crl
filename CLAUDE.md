...
- `eval/identifiability_report.py` — one-page readable R²/MCC/DCI report for a single run vs its untrained floor. Per-factor R² at each factor's assigned pooling, plus an R² ladder showing every factor at every rung (gap/stats/patch). Thin layer over the above; `--self-test` runs torch-free. `extract_reprs` (torch) + `score_extracted` (metric rules) are split so `score_run` (checkpoint/floor) and `score_model_live` (in-training) share one scorer; `report_scalars` flattens the tables to TB tags. Training logs them under `identifiability/*` via `--identifiability-every` — these are NOT the same measurement as `selection/*` (different split, probe width, pooling and quantity), see the 8 Sep 2026 changelog entry.
