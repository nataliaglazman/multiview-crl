...
#: Probe kinds whose multi-output fit is column-wise identical to a single-target fit,
#: and may therefore be batched.  Only ridge qualifies: ``alpha_per_target=True`` picks
#: each column's alpha from one shared GCV decomposition, so the batch changes what is
#: computed once, not what any column scores.  Everything else shares a fitted quantity
#: across columns -- one ``GridSearchCV`` alpha for every kernel target, one network for
#: every MLP target -- which makes a target's score depend on which OTHER targets rode
#: along.  That is not an approximation to fix later: with permutation nulls in the batch
#: it makes ``n_null`` move the real score.  Measured on 120x12 Gaussian features, one
#: real target and one permuted copy, 3-fold CV at seed 0: kernel 0.852159 separate vs
#: 0.709798 batched, MLP 0.936418 vs 0.524527.  Ridge agreed to < 5e-15.
BATCHABLE_PROBES = ("ridge",)


def _make_multi_regressor(kind, seed):
    """Multi-output twin of ``_make_regressor``, for the kinds in ``BATCHABLE_PROBES``.
    ...
    probe column-by-column while decomposing ``X`` only once.
    """
    if kind == "ridge":
        return RidgeCV(alphas=(0.01, 0.1, 1.0, 10.0, 100.0, 1000.0), alpha_per_target=True)
    raise ValueError(f"{kind!r} is not batchable; fit it one target at a time")
...
def cv_probe_r2_multi(X, Y, n_splits=5, seeds=(0, 1, 2), kind="ridge"):
    ...
    Column ``j`` equals :func:`cv_probe_r2` on ``Y[:, j]`` for EVERY ``kind`` — that
    equality is the contract, and it is what lets one script batch its probes and another
    fit them one at a time while their numbers stay on the same scale.  It is enforced by
    routing each kind through the path that preserves it:

    * ``kind in BATCHABLE_PROBES`` — one multi-output fit per (seed, fold).  The
      ``StandardScaler`` + ridge decomposition of ``X[train]`` is computed once and reused
      across all targets, which is the saving that matters when many factors and their
      permutation nulls share one ``X`` (``eval.run_dci_compare._score_block``,
      ``eval.dinov3_identifiability.factor_recovery``).
    * every other kind — one single-target fit per (seed, fold, target), because a shared
      ``GridSearchCV`` alpha or a shared MLP would let the other targets in the batch move
      this column's score.  The scaler is still fitted once per fold, since it does not
      depend on the target; only the estimator is refitted.  This costs one fit per target
      instead of one per fold, so a kernel or MLP sweep is roughly ``T`` times the work a
      batched one appeared to be — that apparent saving was the bug, not a feature.  Ridge
      stays the default and the kind to use for cross-script comparisons.
    ...
    batched = kind in BATCHABLE_PROBES
    ...
    for si, seed in enumerate(seeds):
        ...
        for fi, (tr, te) in enumerate(kf.split(X)):
            ...
            if batched:
                ...
                model.fit(Xtr, Y[tr])
                pred = np.asarray(model.predict(Xte)).reshape(len(te), T)
                fold[fi] = r2_score(Y[te], pred, multioutput="raw_values")
            else:
                for t in range(T):
                    model = _make_regressor(kind, seed)
                    # Column kept 2-D, as cv_probe_r2 passes it, so a kind that reacts to
                    # target shape reacts the same way on both paths.
                    model.fit(Xtr, Y[tr, t : t + 1])
                    pred = np.asarray(model.predict(Xte)).reshape(len(te), -1)
                    fold[fi, t] = r2_score(Y[te, t : t + 1], pred, multioutput="variance_weighted")
