...
class CausalConv2d(HelperModule):
    def build(self, in_channel, out_channel, kernel_size, stride=1, padding="downright"):
        assert padding in [
            ...
            "down",
            "causal",
        ], f"Unknown padding type! Got '{padding}'"
    ...
    def forward(self, x):
        x = self.pad(x)
        if self.causal:
            self.conv.weight_v.data[:, :, -1, self.causal :].zero_()  # noqa: E203
        x = self.conv(x)
        return x
...
class CausalAttention(HelperModule):
    def build(self, query_channel, key_channel, channel, nb_heads=8, dropout=0.1):
        self.to_qkv = nn.Linear(query_channel + key_channel + key_channel, channel * 3)
    ...
    def forward(self, q, k):
        ...
        def reshape(x):
            return x.view(batch_size, -1, self.nb_heads, self.head_dim).transpose(1, 2)
        ...
        y = (
            (attn @ v)
            .transpose(1, 2)
            .reshape(batch_size, height, width, self.head_dim * self.nb_heads)
            .permute(0, 3, 1, 2)
        )
        return y
...
class CondResNet(HelperModule):
    def build(self, in_channel, channel, kernel_size, nb_res_blocks):
        ...
        blocks.extend([GatedResBlock(channel, channel, kernel_size) for _ in range(nb_res_blocks)])
...
class PixelSnail(nn.Module):
    def __init__(
        ...
        if nb_cond > 0:
            ...
            self.cond_net = CondResNet(
                nb_cond * cond_embedding_dim,
                cond_res_channel,
                cond_res_kernel,
                nb_cond_res_block,
            )
            ...
            self.cond_in_net = nn.ModuleList(
                [
                    CondResNet(
                        ...
                        cond_embedding_dim,
                        cond_res_kernel,
                        nb_cond_in_res_block,
                    )
                    if nb_cond_in_res_block > 0
                    else nn.Identity()
...
if __name__ == "__main__":
    ...
    cs = [
        torch.LongTensor(1, 6, 6).random_(0, 255),
        torch.LongTensor(1, 12, 12).random_(0, 255),
    ]
    ps = PixelSnail(
        [24, 24],
        512,
