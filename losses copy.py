...
class AdaptiveLoss(object):
    def __init__(self, image_shape, image_device, lambda_reconstruction=1):
        ...
        # Create the loss function ONCE, not on every call
        self._av_loss_fn = AdaptiveVolumeLossFunction(
            image_size=image_shape, device=image_device
        )
        self.lambda_reconstruction = lambda_reconstruction

    def av_loss(self, x):
        return self._av_loss_fn.lossfun(x.permute(0, 2, 3, 4, 1))
    ...
    def __call__(self, network_outputs):
        ...
        summaries[("summaries", "scalar", "Adaptive-Loss")] = adaptive_loss.item()
        summaries[
